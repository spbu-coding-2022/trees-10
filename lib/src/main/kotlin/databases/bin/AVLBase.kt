...
class AVLBase (path: String): Closeable {
    ...
    fun addNode(node : Node<AVLNode<Int, String>>) {
    ...
    fun search(key: Int) : Node<AVLNode<Int, String>>? {
        getNodeByKey.setInt(1, key)
        val res = getNodeByKey.executeQuery()
        if (res.isClosed)
            return null

        return Node(AVLNode(key, res.getString("value")), res.getInt("x"), res.getInt("y"))
    }
    override fun close() {
